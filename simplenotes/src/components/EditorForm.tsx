...
const EditorForm = ({ noteId }: EditorFormProps) => {
  ...
  const loadNoteData = async () => {
    ...
    if(data) {
      console.log(data)
      setInitNote(data[0].content.html);
    };
  }
  ...
  const commitChanges = async (html: string | null) => {
    ...
    const { error } = await supabase.from('notes').update({ content: {"html": html} }).eq('id', 1);
    if(error) console.log(error);
  }
  ...
  return (
    <>
      {initNote === null ? (<p>loading...</p>) : (
        <TrixEditor onChange={handleChange} onEditorReady={handleEditorReady} value={initNote} mergeTags={[]} />
      )}
    </>
  )
};
